...
def _thread_worker(aln_reader, contigs_info, err_mode,
    ...
    try:
        ...
        while not aln_reader.is_eof():
            ...
            profile, aln_errors = _compute_profile(ctg_aln, err_mode, contigs_info[ctg_id].length)
            partition, num_long_bubbles = _get_partition(profile, err_mode)
            ctg_bubbles = _get_bubble_seqs(ctg_aln, err_mode, profile, partition, contigs_info[ctg_id])
